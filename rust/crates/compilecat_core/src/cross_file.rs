...
//!
//! The consumer imports an `@inline` dependency (`import { add } from './math'`).
//! The JS plugin resolves + reads the dependency modules and hands them here; this
...
//!
//! Scope: DIRECT/BLOCK dependencies. Module-scope **declarations** the spliced body
...
//! Deferred: dependencies whose body needs an **imported** dep (would need the import
//! re-resolved relative to the dependency) — those are left un-inlined for now.
//!
//! Name collisions (α-rename, soundness): a needed dependency dep whose name collides
//! with a *different* entity already in the consumer — a consumer-original
//! top-level binding, or a *different* dependency's already-copied dep — would, if
//! copied verbatim, make the spliced body bind to the consumer's entity (a
//! MISCOMPILE: the consumer's value, not the dependency's). On such a *true* collision
...
//! and any other cloned dependency deps that reference it. The rename runs on CLONED
//! dependency material only — the shared parse cache is never mutated. A re-copy of the
//! *same* dependency's *same* dep is a legitimate dedupe (same entity) and reuses the
...
use crate::passes::inline_functions::{
    callsite_inline_spans, classify_block, classify_block_arrow, classify_direct,
    classify_direct_arrow, inline_callsites, inline_with, strip_spans_in_statement,
    top_level_function, BlockCandidate, Candidate,
};

/// A dependency module the consumer imports from.
pub struct Dependency {
    /// The specifier the consumer used to import it (`./math`).
    pub specifier: String,
    /// The dependency's own resolved path — used to rebase the dependency's *relative*
    /// imports when forwarding them into the consumer, and to match this module
    /// as a re-export target. Empty if unknown.
    pub path: String,
    pub code: String,
    /// Resolved `… from '<specifier>'` edges of *this* dependency: each (specifier,
    /// path) lets the core follow a re-export (`export * as vec3 from './vec3'`)
    /// to the dependency at that path without re-implementing module resolution.
...
pub fn transform_cross_file(
    ...
    // Phase 1: parse every dependency once into the (persistent) cache, keyed by path.
    ...
    for d in dependencies {
        cache.get_or_parse(&d.path, &d.code, dependency_source_type(d, source_type));
    }
    ...
    for (local, specifier, imported) in &imports {
        let Some(dependency) = dependencies.iter().find(|d| &d.specifier == specifier) else { continue };
        let dependency_program = cache.get(&dependency.path).expect("cached in phase 1").program();

        // (a/b) callable or object-namespace export of the directly-imported dependency.
        ...
        if let Some((target, target_program)) =
            resolve_namespace_reexport(dependency_program, dependency, dependencies, imported, cache)
        {
        ...
        if let Some((target, target_program, name)) =
            resolve_named_reexport(dependency_program, dependency, dependencies, imported, cache)
        {
            reg.register_named(
                target_program,
                &target.path,
    ...
    for (ns_local, specifier) in &namespaces {
        let Some(dependency) = dependencies.iter().find(|d| &d.specifier == specifier) else { continue };
        let dependency_program = cache.get(&dependency.path).expect("cached in phase 1").program();
        reg.register_namespace(dependency_program, &dependency.path, consumer_path, ns_local, &allocator);
    }

    let mut stats = Stats::default();
    // Cross-file `@inline` targets carry no directive of their own; record the
    // consumer functions that call a dependency BEFORE inlining (the calls vanish
    ...
    // One inline-temp counter for the WHOLE chunk: cross-file dependency inlining
    ...
    if !reg.direct.is_empty() || !reg.block.is_empty() {
        // Dependency AST is spliced into the consumer; its spans index the dependency source,
        // so strip them before codegen builds the consumer sourcemap (no-op otherwise).
        if options.sourcemap {
            reg.strip_dependency_spans();
        }
    ...
    // Cross-file call-site `/* @inline */`: a marked call to an IMPORTED callable
    // (`/* @inline */ vec3.scaleAndAdd(…)` where `vec3` is imported) splices that
    // callee's body at THAT call only — the call-site analogue of the `@flatten`
    // section just above, differing only in scope (one marked call vs a whole
    // host) and applier (`inline_callsites` vs `flatten_into_hosts`). It reuses the
    // identical import resolution (`register_host_called_imports` →
    // `register_export` + dep forwarding), so imported named / namespace-member
    // callees resolve and forward their module deps exactly as `@flatten` does. An
    // UNMARKED call to the same imported callee elsewhere stays a call.
    let callsite_spans = callsite_inline_spans(&program);
    if !callsite_spans.is_empty() {
        // The callees of the marked calls — the imports the call-site pass must
        // resolve (same `fn` / `obj.member` keying as the `@flatten` host scan).
        let host_calls = call_names_in_hosts(&program, &callsite_spans);
        let mut creg = Registrar::new(&program);
        register_host_called_imports(
            &mut creg,
            &host_calls,
            &imports,
        ...
        if !creg.direct.is_empty() || !creg.block.is_empty() {
            if options.sourcemap {
                creg.strip_dependency_spans();
            }
            prepend_imports(&allocator, &mut program, creg.forward);
            insert_after_imports(&allocator, &mut program, creg.hoist);
            stats.inlined += inline_callsites(
                &allocator,
                &mut program,
                &callsite_spans,
                &creg.direct,
            ...
            remove_unused_imports(&allocator, &mut program, &creg.inlined_locals);
        }
    }

    // Cross-module type resolution: resolve each imported type's shape (tuple
    // arity or record field set) from its dependency module's AST (in the cache), so
    ...
    // the dependency programs.
    let sroa_external_shapes = collect_imported_type_shapes(&imports, dependencies, cache);
...
/// Parse source type for a dependency — prefer its real resolved `path` (correct
/// extension even behind an extensionless specifier), then the specifier, then
/// the consumer's type.
fn dependency_source_type(dependency: &Dependency, fallback: SourceType) -> SourceType {
    SourceType::from_path(&dependency.path)
        .or_else(|_| SourceType::from_path(&dependency.specifier))
        .unwrap_or(fallback)
}

/// Register every import (named or namespace-member) that appears in `host_calls`
/// as an inline candidate in `reg`, forwarding each resolved callable's module
/// deps. This is the shared "resolve the imports the host constructs actually
/// call" step for BOTH cross-file paths: `@flatten`/`@optimize` (`host_calls` =
/// the calls inside flatten hosts) and call-site `@inline` (`host_calls` = the
/// callees of the marked calls). The two differ only in HOW they later apply the
/// candidates (`flatten_into_hosts` vs `inline_callsites`); the resolution — named
/// imports (with a `resolve_value_origin` fallback for const-alias / re-export
/// indirections) and namespace-member calls (direct + barrel-reexport shapes) — is
/// identical, so it lives here once. `already_inlined` skips locals a prior phase
/// (the `@inline` pass) already consumed. The library modules are never mutated;
/// only the host's directives drive what is registered.
#[allow(clippy::too_many_arguments)]
fn register_host_called_imports<'a>(
    reg: &mut Registrar<'a>,
    host_calls: &HashSet<String>,
    imports: &[(String, String, String)],
    namespaces: &[(String, String)],
    dependencies: &[Dependency],
    consumer_path: &str,
    already_inlined: &HashSet<String>,
    cache: &'a crate::ModuleCache,
    allocator: &'a Allocator,
) {
    // (1) Named imports the host calls directly (`add(…)` from `import { add }`).
    for (local, specifier, imported) in imports {
        // Skip names a prior phase already inlined, and imports the host never calls.
        if already_inlined.contains(local) || !host_calls.contains(local) {
            continue;
        }
        let Some(dependency) = dependencies.iter().find(|d| &d.specifier == specifier) else { continue };
        let Some(parsed) = cache.get(&dependency.path) else { continue };
        if let Some(export) = find_export(parsed.program(), imported, false) {
            if reg.register_export(
                parsed.program(),
                &dependency.path,
                consumer_path,
                local.clone(),
                ...
                reg.inlined_locals.insert(local.clone());
            }
        } else {
            ...
            if let Some(origin) = resolve_value_origin(imported, dependency, dependencies, cache, 0, &mut visited) {
                if let Some(oparsed) = cache.get(&origin.dependency_path) {
                    if let Some(ValueBinding::Callable(callable)) =
                        local_value_binding(oparsed.program(), &origin.name)
                    {
                        let export = DependencyExport { name: origin.name.clone(), callable };
                        if reg.register_export(
                            oparsed.program(),
    ...
    // only the host's directives drive the inlining.
    let mut ns_targets: Vec<(String, &Dependency)> = Vec::new();
    for (ns_local, specifier) in namespaces {
        if let Some(dependency) = dependencies.iter().find(|d| &d.specifier == specifier) {
            ns_targets.push((ns_local.clone(), dependency));
        }
    }
    for (local, specifier, imported) in imports {
        let Some(barrel) = dependencies.iter().find(|d| &d.specifier == specifier) else { continue };
        let Some(parsed) = cache.get(&barrel.path) else { continue };
        if let Some(src) = reexported_namespace_source(parsed.program(), imported) {
            if let Some(target) = resolve_reexport_target(barrel, dependencies, &src) {
                ns_targets.push((local.clone(), target));
            }
        }
    }
    for (ns_local, target) in ns_targets {
        if already_inlined.contains(&ns_local) {
            continue;
        }
        ...
        for export in find_all_inline_exports(parsed.program(), false) {
            ...
            if reg.register_export(
                ...
                reg.inlined_locals.insert(ns_local.clone());
        ...
        for member in called_members {
            ...
            let mut visited = HashSet::new();
            let Some(origin) = resolve_value_origin(&member, target, dependencies, cache, 0, &mut visited)
            else {
                continue;
            };
            let Some(oparsed) = cache.get(&origin.dependency_path) else { continue };
...
/// Accumulates inline candidates + the dependency deps to copy/forward into the
/// consumer. Its methods resolve a dependency export (callable, object namespace, or
/// re-export target) and register it; the `hoisted` set dedupes copied/forwarded
/// deps across every dependency and export. Reused for the directly-imported dependency and
...
struct Registrar<'a> {
    ...
    /// every dep copied so far. A needed dependency dep whose name is in here but is a
    ...
    /// legit same-dependency dedupe).
    consumer_names: HashSet<String>,
    /// (dependency_path, dependency-original dep name) → the final name that dep was copied
    /// under (its own name, or an α-renamed `<name>$cf<N>`). A re-copy of the same
    /// dependency's same dep reuses this name (legit dedupe); a *different* dependency's dep
...
impl<'a> Registrar<'a> {
    ...
    /// Neutralise the dependency source spans on every piece of dependency material this
    /// registrar will splice into the consumer — the classified candidate bodies
    /// and the forwarded imports / hoisted decls. Those spans index the *dependency's*
    ...
    /// dependency code then maps to the top of the consumer rather than to a bogus offset.
    fn strip_dependency_spans(&mut self) {
    ...
    ///
    /// Soundness: needed dependency deps whose names *truly collide* with a different
    /// consumer / other-dependency entity are α-renamed to fresh `$cf<N>` names,
    /// consistently across the cloned export body + the cloned deps (so the body
    /// binds to the dependency's entity, not the consumer's same-named one). If a
    /// truly-colliding needed name is supplied by a dependency *import* (which we can't
    ...
    fn register_export(
        ...
        let Some(rename) = self.plan_renames(program, dependency_path, &needed) else {
            return false;
        };

        // Classify the export body. With renames, classify a *renamed clone* of
        // the dependency callable so the spliced body references the fresh names; the
        // cache is never touched. Without renames, classify the borrowed dependency
    ...
    /// Decide the final consumer name for each needed dependency dep, detecting true
    ...
    /// `None` to signal a bail (a truly-colliding name supplied by a dependency import,
    ...
    fn plan_renames(
        ...
        for n in needed {
            // Same dependency's same dep already copied (possibly renamed) → reuse the
            // established name. A legit dedupe, never a collision.
            if let Some(final_name) = self.copied.get(&(dependency_path.to_string(), n.clone())) {
                if final_name != n {
                    rename.insert(n.clone(), final_name.clone());
                }
                continue;
            }
            // Not yet copied from this dependency. A true collision is a name already
            // live in the consumer (original binding or a *different* entity's
            // copied dep) — i.e. present in `hoisted` but not as this dependency's `n`.
            ...
            // clone of the dependency *decl* that supplies `n`. If `n` is supplied by a
            // dependency import (no copyable decl), bail — renaming the body alone
            // would leave it unbound / mis-resolved.
            if !dependency_decl_supplies(program, n) {
                return None;
            }
    ...
    fn register_named(
        ...
        for (member, callable) in object_export_methods(program, imported) {
            let export = DependencyExport { name: member.clone(), callable };
            let key = format!("{local}.{member}");
            if self.register_export(program, dependency_path, consumer_path, key, &export, allocator) {
                any = true;
            }
        }
        if any {
...
fn resolve_namespace_reexport<'b, 'd, 'c>(
    ...
    for stmt in &barrel.body {
        ...
        let target = resolve_reexport_target(barrel_dependency, dependencies, e.source.value.as_str())?;
        return Some((target, cache.get(&target.path)?.program()));
    }
    None
}

/// `export { <name> as <imported> } from S` → (target dependency, cached program, the
...
fn resolve_named_reexport<'b, 'd, 'c>(
    ...
    imported: &str,
    cache: &'c crate::ModuleCache,
) -> Option<(&'d Dependency, &'c Program<'c>, String)> {
    for stmt in &barrel.body {
        ...
        for spec in &e.specifiers {
            ...
            let target = resolve_reexport_target(barrel_dependency, dependencies, source.value.as_str())?;
            return Some((
                target,
...
/// The dependency a re-export `from '<specifier>'` points at, via the barrel's
...
fn resolve_reexport_target<'d>(
    ...
    let path = barrel_dependency
        .resolved
        .iter()
        .find(|(s, _)| s.as_str() == specifier)
        .map(|(_, p)| p.as_str())?;
    dependencies.iter().find(|d| d.path.as_str() == path)
}

/// `import { x }` / `import { x as y }` → (local, specifier, imported); a
/// default import (`import x from`) is folded in as imported = `"default"`, so
/// the dependency side resolves it through the same path.
/// Call keys (`fn` or `obj.member`) that `spans` selects — the candidate names a
/// cross-file phase must resolve. Two selection modes, unified so both cross-file
/// paths reuse it:
///   - a top-level statement whose span-start is in `spans` (a `@flatten`/
///     `@optimize` host) → EVERY call inside it (the host inlines all its callees);
///   - an individual call expression whose OWN span-start is in `spans` (a
///     call-site `/* @inline */ foo()`, whose annotated span is the call, not the
///     enclosing statement) → just that call's callee.
///
/// Passing flatten host spans yields the flatten candidate set; passing call-site
/// spans yields the marked-call callees; a union yields both (the frontier scan).
fn call_names_in_hosts(program: &Program, spans: &HashSet<u32>) -> HashSet<String> {
    struct V<'s> {
        spans: &'s HashSet<u32>,
        /// True while inside a matched host statement subtree — every call counts.
        in_host: bool,
        names: HashSet<String>,
    }
    impl<'s> V<'s> {
        fn record(&mut self, call: &CallExpression) {
            match &call.callee {
    ...
    impl<'a> Visit<'a> for V<'_> {
        fn visit_call_expression(&mut self, call: &CallExpression<'a>) {
            // Inside a host: every call. Outside: only a call whose own span is a
            // call-site marker. (A marker call inside a host is covered either way.)
            if self.in_host || self.spans.contains(&call.span.start) {
                self.record(call);
            }
            walk::walk_call_expression(self, call);
        }
    }
    let mut v = V { spans, in_host: false, names: HashSet::new() };
    for stmt in &program.body {
        // A top-level statement in `spans` is a `@flatten` host: visit it whole
        // with `in_host` set so every nested call is a candidate. Otherwise walk it
        // too, collecting only individually-marked call-site calls it contains.
        v.in_host = spans.contains(&stmt.span().start);
        v.visit_statement(stmt);
    }
    v.names
}

/// Whole-program scan for calls to any first-party `@inline`-def name in `names`.
/// Returns the called keys — a direct call `f(…)` yields `"f"`, a namespace-member
/// call `ns.f(…)` yields `"ns.f"` — keyed identically to `call_names_in_hosts` so
/// the frontier's named-import / namespace-member value-seed loops can map them to
/// import edges exactly as they map flatten-host / call-site calls. Only the LEAF
/// name is matched against `names` (a direct `f`, or the member `f` of `ns.f`), so a
/// def named `helper` fires on both `helper(x)` and `vec.helper(x)`. `names` is a
/// small candidate set (the build-start index); the confirming `@inline`-export
/// check lives in the walk, so a spurious name simply resolves to nothing.
fn program_calls_of(program: &Program, names: &[String]) -> HashSet<String> {
    let name_set: HashSet<&str> = names.iter().map(String::as_str).collect();
    struct V<'s> {
        names: &'s HashSet<&'s str>,
        out: HashSet<String>,
    }
    impl<'a> Visit<'a> for V<'_> {
        fn visit_call_expression(&mut self, call: &CallExpression<'a>) {
            match &call.callee {
                ...
                Expression::StaticMemberExpression(m) => {
                    if self.names.contains(m.property.name.as_str()) {
                        if let Expression::Identifier(obj) = &m.object {
                            self.out.insert(format!("{}.{}", obj.name, m.property.name));
                        }
                    }
                }
                _ => {}
    ...
    let mut v = V { names: &name_set, out: HashSet::new() };
    for stmt in &program.body {
        v.visit_statement(stmt);
    }
    v.out
...
/// from its dependency module, keyed by the consumer's local name — the seed for
...
/// oracle's `build_alias_shapes` over the dependency program (so dependency-local alias /
...
/// — the dependency that declares it isn't the directly-imported module, so its
/// alias map won't carry the name. Direct imports + dependency-local chains only.
fn collect_imported_type_shapes(
    ...
    for (local, specifier, imported) in imports {
        // Try every dependency sharing this specifier, taking the first that yields a shape.
        ...
        // alias. Safe because a stripped `.js` never resolves a type; a future dependency
        // that is a real `.ts` declaring a same-named-but-different type would make this
        // order load-bearing — keep type-source dependencies last.
        for dependency in dependencies.iter().filter(|d| &d.specifier == specifier) {
            let mut visited = HashSet::new();
            if let Some(shape) = resolve_type_alias_shape(imported, dependency, dependencies, cache, 0, &mut visited) {
...
/// the `visited` set, which already bounds traversal to O(dependencies × names)).
const MAX_TYPE_REEXPORT_DEPTH: usize = 32;

/// Where a value binding ultimately resolves: the module (`dependency_path`) + the
...
//
// Value inlining and type SROA both walk the SAME dependency module graph (imports,
...
trait ResolveLeaf {
    ...
    /// keep/stop walking (see [`LocalResolution`]). `dependency_path` is supplied so the
    /// value leaf can record the origin module.
    fn local(program: &Program, name: &str, dependency_path: &str) -> LocalResolution<Self::Out>;
}
...
impl ResolveLeaf for ValueLeaf {
    ...
    fn local(program: &Program, name: &str, dependency_path: &str) -> LocalResolution<Origin> {
        match local_value_binding(program, name) {
            Some(ValueBinding::Callable(_)) => {
                LocalResolution::Found(Origin { dependency_path: dependency_path.to_string(), name: name.to_string() })
            }
...
/// declared → NotDeclared`. Never returns `AliasTo`: dependency-local alias / interface
...
/// Resolve the **exported** name `name` of `dependency` to its leaf (`L::Out`), walking
/// the dependency graph under authoritative precedence (A→E above). Generic over the
...
///
/// `visited` memoises `E\0(dependency, name)` (the export-surface role, distinct from
...
fn resolve_export<L: ResolveLeaf>(
    ...
    if !visited.insert(format!("E\u{0}{}\u{0}{}", dependency.path, name)) {
        return None;
    }
    ...
    // is resolved as a LOCAL binding of the dependency (never through its export
    ...
    if name == "default" {
        if let Some(local) = export_default_ident(program) {
            return resolve_local::<L>(&local, dependency, dependencies, cache, depth + 1, visited);
        }
    }
    ...
    match L::local(program, name, &dependency.path) {
        LocalResolution::Found(out) => return Some(out),
        LocalResolution::AliasTo(target) => {
            return resolve_local::<L>(&target, dependency, dependencies, cache, depth + 1, visited);
        }
    ...
    if let Some((source, imported)) = import_binding_source(program, name) {
        return match resolve_reexport_target(dependency, dependencies, &source) {
            Some(target) => resolve_export::<L>(&imported, target, dependencies, cache, depth + 1, visited),
            None => None,
        };
    }
    ...
    for stmt in &program.body {
        ...
        let Some(target) = resolve_reexport_target(dependency, dependencies, e.source.value.as_str()) else {
            continue;
        };
...
///
/// `visited` memoises `L\0(dependency, name)` — a role distinct from the `E\0…`
...
fn resolve_local<L: ResolveLeaf>(
    ...
    if !visited.insert(format!("L\u{0}{}\u{0}{}", dependency.path, local)) {
        return None;
    }
    let program = cache.get(&dependency.path)?.program();

    // (C) A local declaration of `local`.
    match L::local(program, local, &dependency.path) {
        LocalResolution::Found(out) => return Some(out),
        LocalResolution::AliasTo(target) => {
            return resolve_local::<L>(&target, dependency, dependencies, cache, depth + 1, visited);
        }
    ...
    let target = resolve_reexport_target(dependency, dependencies, &source)?;
    resolve_export::<L>(&imported, target, dependencies, cache, depth + 1, visited)
}

/// Resolve the exported value `name` of `dependency` to the module + local name where it
/// is a concrete callable — following const-aliases, imports, and re-exports across
/// the dependency graph, the indirections the direct export scan (`find_export`) can't
...
fn resolve_value_origin(
    ...
    resolve_export::<ValueLeaf>(name, dependency, dependencies, cache, depth, visited)
}

/// Resolve an imported type alias `imported` to its `Shape`, starting at `dependency` and
/// following re-exports across the dependency graph — the shape a package's public type
...
///
/// NB: dependency modules are parsed by their path extension, so the type-source dependency
...
fn resolve_type_alias_shape(
    ...
    resolve_export::<TypeLeaf>(imported, dependency, dependencies, cache, depth, visited)
}
...
fn dependency_inline_spans(dependency: &Program) -> HashSet<u32> {
    crate::passes::directives::annotated_spans(dependency, &["@inline"])
}

/// An inlinable callable dependency export — a `function` declaration / expression or
...
/// A named `@inline` export of a dependency module.
struct DependencyExport<'b, 'a> {
    name: String,
    callable: Callable<'b, 'a>,
}
...
fn inline_exports_of<'b, 'a>(
    ...
    require_inline: bool,
    out: &mut Vec<DependencyExport<'b, 'a>>,
) {
    ...
    if let Some(f) = top_level_function(stmt) {
        if annotated || inline_spans.contains(&f.span.start) {
            if let Some(id) = &f.id {
                out.push(DependencyExport { name: id.name.to_string(), callable: Callable::Func(f) });
            }
        }
        return;
    }
    ...
    if let Some(v) = var_decl {
        for d in &v.declarations {
            ...
            if let Some(callable) = callable_of_init(init) {
                out.push(DependencyExport { name: id.name.to_string(), callable });
            }
        }
    }
}

/// A named (bare or exported) `@inline` callable export of the dependency.
fn find_inline_export<'b, 'a>(dependency: &'b Program<'a>, name: &str) -> Option<DependencyExport<'b, 'a>> {
    find_export(dependency, name, true)
}

/// A named (bare or exported) callable export of the dependency. `require_inline`
...
/// its callees).
fn find_export<'b, 'a>(
    dependency: &'b Program<'a>,
    name: &str,
    require_inline: bool,
) -> Option<DependencyExport<'b, 'a>> {
    find_all_inline_exports(dependency, require_inline).into_iter().find(|e| e.name == name)
}

/// All `@inline` callables the dependency actually *exports*, each under the name a
...
fn find_all_inline_exports<'b, 'a>(
    dependency: &'b Program<'a>,
    require_inline: bool,
) -> Vec<DependencyExport<'b, 'a>> {
    let inline_spans = if require_inline { dependency_inline_spans(dependency) } else { HashSet::new() };
        ...
        match stmt {
            Statement::ExportNamedDeclaration(e) if e.declaration.is_some() => {
                ...
                for local in names {
                    if let Some(c) = callables.get(&local) {
                        out.push(DependencyExport { name: local, callable: *c });
                    }
                }
            }
                ...
                if !require_inline || inline_spans.contains(&stmt.span().start) =>
            {
                ...
                if let Some(c) = default_callable(&e.declaration, &callables) {
                    out.push(DependencyExport { name: "default".to_string(), callable: c });
                }
            }
            _ => {}
        }
...
/// `export default x` indirection resolved against the dependency's @inline bindings.
...
/// Exported name → local binding name across the dependency's export surface
...
fn export_name_to_local(dependency: &Program) -> std::collections::HashMap<String, String> {
...
fn object_export_methods<'b, 'a>(
    ...
    let inline_spans = dependency_inline_spans(dependency);
    ...
    let locals = export_name_to_local(dependency);
...
/// Forward the dependency imports + copy the dependency decls that a needed-set requires
/// into the consumer accumulators. Shared by named and namespace inlining; the
/// `hoisted` set dedupes across dependencies and functions.
#[allow(clippy::too_many_arguments)]
fn pull_dependency_deps<'a>(
    dependency_program: &Program<'a>,
    dependency_path: &str,
    consumer_path: &str,
    needed: &HashSet<String>,
    ...
    // Forward needed dependency imports, rebasing relative specifiers to the consumer.
    ...
    for stmt in &dependency_program.body {
        let Statement::ImportDeclaration(imp) = stmt else { continue };
        let rebased = rebase_specifier(&imp.source.value, dependency_path, consumer_path);
        if let Some(fwd) = forward_import(imp, needed, &rebased, allocator, hoisted) {
            forward.push(fwd);
        }
    }
    // Copy needed dependency decls (dependency source order preserves dependency order).
    for stmt in &dependency_program.body {
        let mut names = HashSet::new();
        collect_top_level_binding_names(stmt, &mut names);
        ...
        // copied from THIS dependency (a legit dedupe of the same entity). A name
        // present in `hoisted` but not in `copied` for this dependency was a true
        // collision — it gets a fresh name via `rename`, so we still copy here.
        let all_dedup = names.iter().all(|n| {
            copied.contains_key(&(dependency_path.to_string(), n.clone())) || !needed.contains(n)
        });
        if all_dedup {
            continue; // this dependency's contribution already present
        }
        if let Some(h) = hoist_decl_renamed(stmt, rename, allocator) {
            for n in &names {
                ...
                copied.insert((dependency_path.to_string(), n.clone()), final_name);
            }
            hoist.push(h);
        }
    }
...
/// Clone a dependency decl for hoisting (unwrapping any `export`), then α-rename every
...
/// the clone only; the dependency program is never mutated.
...
/// Does a hoistable (non-import) dependency decl bind `name`? Used to decide whether a
...
fn dependency_decl_supplies(dependency: &Program, name: &str) -> bool {
    for stmt in &dependency.body {
...
/// Clone the dependency callable into the arena, α-rename `rename` consistently inside
/// the clone, and return a `Callable` over the clone (the dependency cache is never
...
fn apply_renames_function<'a>(
    ...
    for (from, to) in rename {
        let to_a: &'a str = allocator.alloc_str(to);
        // A param/local of the dependency fn that shadows `from` blocks the rename of
...
/// Scope-aware renamer over CLONED dependency material: rewrites every free occurrence
...
/// Dependency module-scope names the export's body needs — transitively (a needed
/// const may reference another const / helper fn). Params, the export's own
/// name (recursion), and plain globals are excluded.
fn needed_module_names(dependency: &Program, export: &DependencyExport) -> HashSet<String> {
...
/// Forward a dependency import into the consumer, keeping only the specifiers the
...
/// Rewrite a dependency import specifier so it resolves the same target from the
...
fn rebase_specifier(spec: &str, dependency_path: &str, consumer_path: &str) -> String {
    let is_relative =
        spec.starts_with("./") || spec.starts_with("../") || spec == "." || spec == "..";
    if !is_relative || dependency_path.is_empty() {
        return spec.to_string();
    }
    // Resolve `spec` against the dependency's directory.
    let mut target = parent_components(dependency_path);
...
/// Clone a dependency module-scope declaration for copying into the consumer.
...
/// The specifiers the dependency BFS should follow *from this one module* — computed
/// by PARSING the module (oxc), the AST-correct replacement for the plugin's
/// brittle dependency-edge regexes (`reexportedImportSources` / `EXPORT_FROM` /
...
pub fn dependency_edges(code: &str, source_type: SourceType) -> Vec<String> {
    let allocator = Allocator::default();
...
/// x }` clause of `program` — the namespace-barrel gate for [`dependency_edges`] step (2),
...
// ── Demand-driven dependency gathering: the resolution frontier ───────────────────
//
// The eager gather (walk the whole reachable module graph, resolve+read every
// module, hand the core a COMPLETE dependency set) dominates build time — we resolve
// the world to answer a handful of name-precise queries. [`resolution_frontier`]
// inverts that: given the host and the dependencies gathered SO FAR, it reports only
// the module edges the host actually still needs, so the plugin can gather them
// on demand via a fixpoint loop and then call the UNCHANGED [`transform_cross_file`]
// with that minimal set.
//
// STATELESS by design: the plugin calls it repeatedly against a growing `provided`
// set until it returns empty (each still-missing edge names ONE module to read).
// The walk mirrors the SAME authoritative precedence (A→E) as [`resolve_export`] /
// [`resolve_local`] — it delegates every AST/binding decision to the same shared
// helpers (`resolve_named_reexport`, `sourceless_export_local`, `local_value_binding`,
// `import_binding_source`, `export_default_ident`, `resolve_reexport_target`,
// `find_all_inline_exports`, `reexported_namespace_source`), so there is no forked
// binding logic — only the traversal loop is parallel. It is parallel rather than a
// sink threaded through `resolve_export` because the real walker's `None` returns are
// its load-bearing STOP points (the "do NOT add a fall-through" contract, locked by
// regression tests): threading a "and by the way, which edge was missing?" collector
// through those arms would obscure that contract for a caller whose `provided` set is
// deliberately incomplete. The frontier's job is exactly to distinguish "unreachable
// because a dependency is absent" (emit a request) from a real resolution failure — a
// distinction the real walker neither makes nor needs.

/// Which resolution a request feeds — the plugin resolves the two differently. A
/// VALUE need is an inlined callable that lives in a runtime `.js`; a TYPE need is
/// an SROA shape that lives in a `.d.ts` (a types-only module often has no `.js` at
/// all), so the plugin MUST know which to resolve to.
...
/// A module edge the host still needs but that isn't reachable within the dependencies
/// gathered so far: `(specifier, from_path, kind)` — the plugin resolves `specifier`
/// relative to the module at `from_path` (as a runtime module for `Value`, a type
/// module for `Type`), reads it, and adds it as a dependency.
type FrontierRequest = (String, String, FrontierKind);

/// Accumulates deduped, first-seen-order missing edges as the frontier walk hits
/// hops it can't follow within `provided`. `kind` tags every edge emitted during
/// the current phase (Value while walking value seeds, Type while walking type
/// seeds) — the whole walk delegates its emits through here, so a single field
/// carries the kind without threading it through every recursive call.
struct Frontier {
    out: Vec<FrontierRequest>,
    seen: HashSet<FrontierRequest>,
    /// Memoises `(kind, role, path, name)` walk nodes so a dense re-export graph
    /// can't re-expand shared nodes (mirrors the `visited` set in [`resolve_export`]).
    /// The kind is part of the key so the SAME node walked once as a Value and once
    /// as a Type is not wrongly deduped away — a `.js` that terminates a value walk
    /// must still be walkable as a type (and vice versa).
    visited: HashSet<String>,
    /// The kind tagging emits/visited-keys during the current phase.
    kind: FrontierKind,
}
...
impl Frontier {
    fn new() -> Self {
        Frontier {
            ...
            seen: HashSet::new(),
            visited: HashSet::new(),
            kind: FrontierKind::Value,
        }
    }

    /// Record a missing edge (once per (specifier, from_path, kind), first-seen order).
    fn emit(&mut self, specifier: &str, from_path: &str) {
        let req = (specifier.to_string(), from_path.to_string(), self.kind);
        if self.seen.insert(req.clone()) {
            self.out.push(req);
        }
    }

    /// Guard a walk node (kind-scoped); `false` means already visited this kind.
    fn enter(&mut self, role: char, path: &str, name: &str) -> bool {
        self.visited.insert(format!("{:?}\u{0}{role}\u{0}{path}\u{0}{name}", self.kind))
    }
}

/// Compute the module edges the host still needs but that aren't reachable within
/// the dependencies gathered so far. STATELESS: call repeatedly with the growing
/// `provided` set until it returns empty, then hand the assembled set to the
/// UNCHANGED [`transform_cross_file`].
///
/// The returned list is deduped and first-seen-order stable; each entry names ONE
/// module the plugin should resolve (`specifier` relative to `from_path`) and read
/// as a further dependency. It follows the same name-precise needs and the same
/// authoritative re-export precedence the inliner does, so it never asks for a
/// module the transform won't consult, and (once every edge it names is provided)
/// it returns empty — the fixpoint terminates.
///
/// A directive-less host (no `@optimize`/`@flatten`/`@sroa`) has no needs and
/// returns empty immediately — UNLESS it CALLS a first-party `@inline`-def name
/// (`inline_def_names`, the build-start index of `/* @inline */ export function
/// NAME` definitions across in-scope src). Such a call is a value need with no
/// directive of its own — the C++-`inline`-style "mark the def once, inline every
/// call" ergonomic — so its defining module gets gathered and the UNCHANGED
/// `transform_cross_file` `require_inline` path inlines the call.
pub fn resolution_frontier(
    host_id: &str,
    host_code: &str,
    provided: &[Dependency],
    inline_def_names: &[String],
    options: &TransformOptions,
) -> Vec<FrontierRequest> {
    let allocator = Allocator::default();
    let source_type = options.source_type();
    let program = parse_program(&allocator, host_code, source_type);

    // A host with no optimization directive drives no inlining and no cross-module
    // type-aware SROA → no dependency needs. (`@optimize` implies `@flatten`+`@sroa`.)
    // A call-site `/* @inline */` of an IMPORTED callee is a value need too — it
    // inlines that callee's module — so it also fires the frontier even absent any
    // `@flatten`/`@sroa`.
    let flatten_spans = passes::inline_functions::collect_flatten_spans(&program);
    let callsite_spans = callsite_inline_spans(&program);
    let has_sroa =
        !crate::passes::directives::annotated_spans(&program, &["@sroa", "@optimize"]).is_empty();
    // Whole-program calls to first-party `@inline`-def names (`def_calls` = the
    // subset of `inline_def_names` this host actually calls, direct or `ns.member`).
    // A directive-LESS host that calls one still fires the frontier: that def's
    // module must be gathered so the `require_inline` path can inline the call.
    let def_calls = if inline_def_names.is_empty() {
        HashSet::new()
    } else {
        program_calls_of(&program, inline_def_names)
    };
    if flatten_spans.is_empty() && callsite_spans.is_empty() && !has_sroa && def_calls.is_empty() {
        return Vec::new();
    }

    // Parse every provided dependency once so the shared binding/precedence helpers can
    // walk their programs (they take `&Program`; the cache owns the parses).
    ...
    // ── Step A: the host's NAME-PRECISE value needs ─────────────────────────────
    // Value phase: every edge emitted here is a runtime-`.js` need. Scanned over
    // `flatten_spans ∪ callsite_spans` — the calls inside `@flatten`/`@optimize`
    // hosts AND the individually-marked `/* @inline */` calls — so a marked call to
    // an imported callee becomes a value need and its module gets gathered, exactly
    // as a flatten-host call does. Unioned with `def_calls` — the whole-program calls
    // to first-party `@inline`-def names — so a directive-LESS host that calls such a
    // def also seeds its defining module (the "mark once, inline everywhere"
    // ergonomic). All three map called names → `(specifier, imported)` via the same
    // `call_names_in_hosts` + walk below.
    frontier.kind = FrontierKind::Value;
    let value_span_hosts: HashSet<u32> =
        flatten_spans.iter().chain(callsite_spans.iter()).copied().collect();
    if !value_span_hosts.is_empty() || !def_calls.is_empty() {
        let mut host_calls = call_names_in_hosts(&program, &value_span_hosts);
        host_calls.extend(def_calls.iter().cloned());

        // (1) Named imports the host calls directly (`add(…)` from `import { add }`).
        // Follow the imported name through the dependency graph as a VALUE.
        for (local, specifier, imported) in &imports {
            if !host_calls.contains(local) {
                continue;
            }
            frontier_value_seed(specifier, imported, host_id, provided, &cache, &mut frontier);
        }

        // (2) Namespace-member calls (`vec3.member(…)`): reach `member` through the
        // namespace object `vec3`. `vec3` is either a direct `import * as vec3 from S`,
        // or a barrel re-export (`import { vec3 } from B` where B does `import * as vec3
        // from S; export { vec3 }`). Resolve `vec3` to the module whose exports are its
        // members, then walk each host-called member as a VALUE there. Mirrors the
        // `ns_targets` construction in `transform_cross_file`.
        for (ns_local, specifier) in &namespaces {
            frontier_namespace_members(
                ns_local, specifier, host_id, &host_calls, provided, &cache, &mut frontier,
            );
        }
        for (local, specifier, imported) in &imports {
            // A named import that is itself a re-exported namespace barrel: the seed
            // edge to the barrel is the same value seed already emitted in (1) when the
            // host calls `local.member`. Follow the barrel → namespace-source hop here.
            let prefix = format!("{local}.");
            if !host_calls.iter().any(|k| k.starts_with(&prefix)) {
                continue;
            }
            // Reach the barrel dependency (by specifier, like the top-level import loop).
            let Some(barrel) = provided.iter().find(|d| &d.specifier == specifier) else {
                frontier.emit(specifier, host_id);
                continue;
            };
            let Some(parsed) = cache.get(&barrel.path) else { continue };
            // The barrel surfaces `vec3` as a namespace whose members live in `S`,
            // either as `export * as vec3 from S` (its own source) or as `import * as
            // vec3 from S; export { vec3 }` (a sourceless clause). Both mirror the
            // `ns_targets` construction in `transform_cross_file` (steps (c) / the
            // `reexported_namespace_source` fallback).
            let ns_src = namespace_reexport_source(parsed.program(), imported)
                .or_else(|| reexported_namespace_source(parsed.program(), imported));
            if let Some(src) = ns_src {
                match resolve_reexport_target(barrel, provided, &src) {
                    Some(target) => frontier_namespace_members_at(
                        local, target, &host_calls, provided, &cache, &mut frontier,
                    ),
                    None => frontier.emit(&src, &barrel.path),
                }
            }
        }
    }

    // ── Step A: the host's imported TYPE needs ──────────────────────────────────
    // The set `collect_imported_type_shapes` operates on: every named import,
    // resolved as a TYPE. Over-inclusion (a value import tried as a type) is safe —
    // its edges get provided and then resolve to nothing, and stop being requested.
    // Type phase: every edge emitted here is a `.d.ts` need.
    frontier.kind = FrontierKind::Type;
    for (_local, specifier, imported) in &imports {
        frontier_type_seed(specifier, imported, host_id, provided, &cache, &mut frontier);
    }

    frontier.out
}

/// Seed a VALUE need at the host's `import … from 'specifier'` edge and walk it.
/// The directly-imported dependency is matched by SPECIFIER (as the top-level import
/// loops in `transform_cross_file` do); deeper re-export hops are path-based.
fn frontier_value_seed(
    specifier: &str,
    imported: &str,
    host_id: &str,
    provided: &[Dependency],
    ...
    match provided.iter().find(|d| d.specifier == specifier) {
        Some(dependency) => frontier_walk_export::<ValueLeaf>(imported, dependency, provided, cache, frontier),
        None => frontier.emit(specifier, host_id),
    }
}

/// Seed a TYPE need at the host's import edge and walk it (the SROA-shape side).
///
/// Unlike the value seed's `.find`, this walks EVERY dependency sharing the specifier —
/// mirroring `collect_imported_type_shapes`. The plugin provides a package as BOTH
/// its runtime `.js` and its `.d.ts` under one specifier; `.find` would grab the
/// `.js` (which carries no types) and miss the `.d.ts` that declares the shape. The
/// `.js` walk terminates empty — its `export * as ns` namespace re-exports are
/// correctly skipped by the TYPE precedence (a namespace binding is not a flat type
/// name) — while the `.d.ts` walk finds the shape or requests the module it lives in.
/// The frontier's dedup unions the two walks' emitted requests.
fn frontier_type_seed(
    specifier: &str,
    imported: &str,
    host_id: &str,
    provided: &[Dependency],
    ...
    let mut any = false;
    for dependency in provided.iter().filter(|d| d.specifier == specifier) {
        any = true;
        frontier_walk_export::<TypeLeaf>(imported, dependency, provided, cache, frontier);
    }
    if !any {
        frontier.emit(specifier, host_id);
    }
...
/// Resolve the namespace object `ns_local` (a direct `import * as ns from
/// specifier`) to its target module, then walk each host-called `ns.member`
/// there. Emits the seed edge if the target module isn't provided yet.
#[allow(clippy::too_many_arguments)]
fn frontier_namespace_members(
    ns_local: &str,
    specifier: &str,
    host_id: &str,
    host_calls: &HashSet<String>,
    ...
    match provided.iter().find(|d| d.specifier == specifier) {
        Some(target) => {
            frontier_namespace_members_at(ns_local, target, host_calls, provided, cache, frontier)
        }
        None => frontier.emit(specifier, host_id),
    }
}

/// Walk each host-called `ns_local.member` as a VALUE within `target` (the module
/// whose exports are the namespace's members). Members surfaced directly by
/// `find_all_inline_exports` and members bound to an indirection (`const set =
/// set$1`, a re-export) are both followed — mirroring `transform_cross_file`'s
/// direct-scan + `resolve_value_origin` fallback for namespace members.
fn frontier_namespace_members_at(
    ns_local: &str,
    target: &Dependency,
    host_calls: &HashSet<String>,
    provided: &[Dependency],
    ...
    let prefix = format!("{ns_local}.");
    for key in host_calls {
        let Some(member) = key.strip_prefix(&prefix) else { continue };
        frontier_walk_export::<ValueLeaf>(member, target, provided, cache, frontier);
    }
}

/// Walk the EXPORTED name `name` of `dependency` under the same authoritative precedence
/// (A→E) as [`resolve_export`], but in collect-missing mode: every hop that must
/// step through a specifier to a module NOT reachable within `provided` records that
/// `(specifier, path)` edge instead of stopping silently. `dependency` is always a module
/// already in `provided` (we only recurse into provided modules). Generic over the
/// leaf `L` so value and type needs share the traversal.
fn frontier_walk_export<L: ResolveLeaf>(
    name: &str,
    dependency: &Dependency,
    provided: &[Dependency],
    cache: &crate::ModuleCache,
    ...
    if !frontier.enter('E', &dependency.path, name) {
        return;
    }
    let Some(parsed) = cache.get(&dependency.path) else { return };
    let program = parsed.program();

    // (A) Sourced named re-export `export { L as name } from S`.
    // `resolve_named_reexport` returns the target dependency only when it's reachable in
    // `provided`; when it can't find it, the edge is missing → emit it and stop.
    if let Some(spec) = sourced_named_reexport_spec(program, name) {
        match resolve_reexport_target(dependency, provided, &spec.0) {
            Some(target) => frontier_walk_export::<L>(&spec.1, target, provided, cache, frontier),
            None => frontier.emit(&spec.0, &dependency.path),
        }
        return;
    }

    // (A′) `export default <ident>` indirection — resolve the identifier as a LOCAL.
    if name == "default" {
        if let Some(local) = export_default_ident(program) {
            frontier_walk_local::<L>(&local, dependency, provided, cache, frontier);
            return;
        }
    }
    ...
    // (C) A local declaration of `name` — authoritative, shadows imports / `export *`.
    // A usable or opaque leaf terminates here (no missing edge); an alias is followed.
    match L::local(program, name, &dependency.path) {
        LocalResolution::Found(_) => return,
        LocalResolution::AliasTo(target) => {
            frontier_walk_local::<L>(&target, dependency, provided, cache, frontier);
            return;
        }
        LocalResolution::DeclaredOpaque => return,
        LocalResolution::NotDeclared => {}
    }

    // (D) `import { imp as name } from S` — authoritative over `export *`.
    if let Some((source, imported)) = import_binding_source(program, name) {
        match resolve_reexport_target(dependency, provided, &source) {
            Some(target) => frontier_walk_export::<L>(&imported, target, provided, cache, frontier),
            None => frontier.emit(&source, &dependency.path),
        }
        return;
    }

    // (E) Bare `export * from S` — reached only when nothing above shadows it. Every
    // wildcard source is a place `name` could live, so each unreachable one is a
    // request; each reachable one is walked (the first that resolves wins in the real
    // walker, but here we can't know which without the module, so we walk/emit all).
    for stmt in &program.body {
        let Statement::ExportAllDeclaration(e) = stmt else { continue };
        if e.exported.is_some() {
            continue; // `export * as ns from S` — a namespace binding, not a flat name
        }
        let src = e.source.value.as_str();
        match resolve_reexport_target(dependency, provided, src) {
            Some(target) => frontier_walk_export::<L>(name, target, provided, cache, frontier),
            None => frontier.emit(src, &dependency.path),
        }
    }
}

/// Walk a LOCAL binding `local` of `dependency` (a local decl or an import) in
/// collect-missing mode — the frontier twin of [`resolve_local`]. Steps (C)→(D):
/// a local name is NOT resolved through this module's export surface / `export *`.
fn frontier_walk_local<L: ResolveLeaf>(
    local: &str,
    dependency: &Dependency,
    provided: &[Dependency],
    cache: &crate::ModuleCache,
    ...
    if !frontier.enter('L', &dependency.path, local) {
        return;
    }
    let Some(parsed) = cache.get(&dependency.path) else { return };
    let program = parsed.program();
    ...
    // (D) Otherwise it's an import binding → follow its source's export surface.
    if let Some((source, imported)) = import_binding_source(program, local) {
        match resolve_reexport_target(dependency, provided, &source) {
            Some(target) => frontier_walk_export::<L>(&imported, target, provided, cache, frontier),
            None => frontier.emit(&source, &dependency.path),
        }
...
/// `export * as <imported> from S` in `program` → `S` — the specifier-side twin of
/// [`resolve_namespace_reexport`] (which resolves the target dependency). The frontier
/// needs the raw specifier so it can report it missing when the target isn't
/// provided. Distinct from `reexported_namespace_source` (the sourceless `import *
/// as ns; export { ns }` barrel shape), which the caller also tries.
fn namespace_reexport_source(program: &Program, imported: &str) -> Option<String> {
    for stmt in &program.body {
        let Statement::ExportAllDeclaration(e) = stmt else { continue };
        let Some(exported) = &e.exported else { continue }; // bare `export *` — not a namespace
        if exported.name().as_str() == imported {
            return Some(e.source.value.to_string());
        }
    }
    None
}

/// `export { L as name } from S` in `program` → (S, L) — the sourced named-reexport
/// clause matching exported `name`. The specifier-side twin of
/// [`resolve_named_reexport`] (which resolves the target dependency); the frontier needs
/// the raw specifier `S` so it can report it missing when the target isn't provided.
fn sourced_named_reexport_spec(program: &Program, name: &str) -> Option<(String, String)> {
    for stmt in &program.body {
        let Statement::ExportNamedDeclaration(e) = stmt else { continue };
        let Some(source) = &e.source else { continue };
        for spec in &e.specifiers {
            if spec.exported.name().as_str() == name {
                return Some((source.value.to_string(), spec.local.name().to_string()));
            }
        }
    }
    None
...
mod tests {
    ...
    fn run(consumer: &str, dependencies: &[(&str, &str)]) -> String {
        let dependencies: Vec<Dependency> = dependencies
            .iter()
            .map(|(s, c)| Dependency {
                specifier: (*s).into(),
                // The cache keys on path, so give each dependency a unique one
            ...
            .collect();
        let mut cache = crate::ModuleCache::new();
        transform_cross_file(consumer, &dependencies, &opts(), &mut cache).code
    }

    #[test]
    fn cache_parses_a_dependency_once_across_consumers() {
        // Two consumers importing the same dependency (shared cache) → parsed once.
        let dependency =
            "/* @inline */ export function add(a: number, b: number): number { return a + b; }";
        let dependencies = vec![Dependency {
            specifier: "./m".into(),
            path: "/m.ts".into(),
            code: dependency.into(),
            resolved: Vec::new(),
        }];
        let mut cache = crate::ModuleCache::new();
        ...
        let o1 = transform_cross_file(c1, &dependencies, &opts(), &mut cache).code;
        let o2 = transform_cross_file(c2, &dependencies, &opts(), &mut cache).code;
        assert!(o1.contains("x + 1") && o2.contains("y + 2"), "both inlined:\n{o1}\n{o2}");
        assert_eq!(cache.parse_count(), 1, "dependency parsed once across two consumers");
    }

    #[test]
    fn resolves_imported_tuple_type_for_sroa() {
        // `Vec3` is defined+exported in the dependency; the consumer imports it and
...
/* @sroa */ export function f(): number { const v: Vec3 = mk(); v[0] = v[1] + v[2]; return v[0]; }"#;
        let dependency = "export type Vec3 = [number, number, number];";
        let out = run(consumer, &[("./math", dependency)]);
        assert!(
    ...
    #[test]
    fn resolves_imported_type_through_dependency_local_alias_chain() {
        // The dependency exports an alias of a local alias of a tuple — the oracle
        // follows the chain in the dependency program.
        let consumer = r#"import { Vec2 } from "./math";
/* @sroa */ export function f(): number { const v: Vec2 = mk(); return v[0] + v[1]; }"#;
        let dependency = "type Pair = [number, number];\nexport type Vec2 = Pair;";
        let out = run(consumer, &[("./math", dependency)]);
    ...
    fn resolves_imported_type_through_wildcard_reexport_barrel() {
        ...
        // must follow the wildcard re-export across the dependency graph to recover the
...
/* @sroa */ export function f(): number { const v: X = mk(); v[0] = v[1]; return v[0]; }"#;
        let dependencies = vec![
            ...
            barrel_dependency("./a.js", "/p/a.d.ts", "export type X = (n: number) => number;", &[]),
            barrel_dependency("./b.js", "/p/b.d.ts", "export type X = [number, number];", &[]),
        ];
        let out = transform_cross_file(consumer, &dependencies, &opts(), &mut crate::ModuleCache::new()).code;
        assert!(
            out.contains("v[1]") && !out.contains("v_0"),
    ...
    fn resolves_imported_type_through_sourceless_rename_clause() {
        // Bonus of the unified walker: a dependency declares a local tuple type and
...
/* @sroa */ export function f(): number { const v: Vec2 = mk(); return v[0] + v[1]; }"#;
    ...
    fn dense_reexport_graph_does_not_blow_up() {
        ...
        // full traversal. Without the `(dependency, name)` visited memo this re-expands
...
/* @sroa */ export function f(): number { const v: Quat = mk(); v[0] = v[1] + v[2] + v[3]; return v[0]; }"#;
    ...
    fn resolves_imported_object_type_alias_for_sroa() {
        // The dependency exports a `type X = { … }` object-type alias (not an
...
/* @sroa */ export function f(): number { const v: Vec3 = mk(); v.x = v.y + v.z; return v.x; }"#;
        let dependency = "export type Vec3 = { x: number; y: number; z: number };";
        let out = run(consumer, &[("./math", dependency)]);
        assert!(
    ...
    fn cross_file_flatten_inlines_imported_callee() {
        ...
        let dependency = "export function helper(a: number): number { return a * 2; }";
        let out = run(consumer, &[("./m", dependency)]);
    ...
    fn cross_file_flatten_inlines_direct_namespace_member() {
        ...
        let dependency = "export function add(a: number, b: number): number { return a + b; }";
        let out = run(consumer, &[("./m", dependency)]);
...
/* @optimize */ export function host(o: number[]): number[] { return set(o, 1, 2, 3, 4); }"#;
    ...
    fn cross_file_rebound_import_forwards_origin_module_deps() {
        ...
        // resolver registers against the origin dependency so `pull_dependency_deps` rebases
...
/* @optimize */ export function host(o: number[]): number[] { return quat.set(o, 9); }"#;
    ...
    fn cross_file_flatten_inlines_default_reexport_of_imported_callable() {
        ...
        // `@optimize` host calls a DEFAULT-imported binding whose dependency re-exports
...
/* @optimize */ export function host(x: number): number { return foo(x, 1); }"#;
    ...
    fn cross_file_flatten_direct_default_function_still_inlines() {
        ...
        let consumer = r#"import foo from "./m";
/* @optimize */ export function host(x: number): number { return foo(x, 1); }"#;
        let dependency = "export default function add(a, b) { return a + b; }";
        let out = run(consumer, &[("./m", dependency)]);
...
/* @optimize */ export function host(x: number): number { return foo(x, 1); }"#;
    ...
    fn cross_file_inline_with_sourcemap_has_no_out_of_range_spans() {
        // Cross-file inlining splices dependency AST — whose spans index the DEPENDENCY source —
        // into the consumer. With a sourcemap enabled, codegen maps every node's span
        // against the CONSUMER source; a dependency span past the consumer's length is out of
        // range (oxc `debug_assert`s in `add_source_mapping_for_name`, and emits a wrong
        // mapping in release). A *named* dependency node must survive inlining to hit the
        // named-mapping assert: here the inlined `add` body calls a copied helper
        // `padHelper`, whose reference sits (in dependency coordinates) well past the short
...
/* @optimize */ export function h(x: number): number { return add(x, 1); }"#;
        let dependency = "export function add(a, b) { /* padding padding padding padding padding padding padding padding padding to push the Math reference past the consumer length */ return a + b + Math.floor(a); }";
        let mut o = opts();
        o.sourcemap = true;
        let dependencies = vec![barrel_dependency("./m", "/p/m.js", dependency, &[])];
        let out = transform_cross_file(consumer, &dependencies, &o, &mut crate::ModuleCache::new());
        assert!(!out.code.contains("add("), "call inlined:\n{}", out.code);
        // `Math` is a non-inlinable global identifier — it survives inlining as a named
        // node carrying a dependency span (past the consumer length).
...
/* @optimize */ export function host(o: number[]): number[] { return set(o, 1, 2, 3, 4); }"#;
    ...
    fn cross_file_unfollowable_import_shadows_wildcard_no_miscompile() {
        // `set` is imported from a module not in the dependency set (out of scope / unresolved).
...
/* @optimize */ export function host(x) { return vec3.add(x, 1); }"#;
        ...
        let dependencies = vec![
            barrel_dependency("./ns", "/n/ns.js", barrel, &[("./vec3.js", "/n/vec3.js")]),
            barrel_dependency("./vec3.js", "/n/vec3.js", vec3, &[]),
        ];
        let out =
            transform_cross_file(consumer, &dependencies, &opts(), &mut crate::ModuleCache::new()).code;
    ...
    #[test]
    fn cross_file_callsite_inlines_marked_imported_call() {
        // A call-site `/* @inline */` of an IMPORTED (plain, non-@inline) function
        // splices that callee's body at the marked call only — the cross-file
        // call-site path. An UNMARKED call to the same import elsewhere stays a
        // call, and the import survives because that unmarked call still needs it.
        let consumer = r#"import { add } from "./m";
export function f(x: number): number { return /* @inline */ add(x, 1); }
export function g(y: number): number { return add(y, 2); }"#;
        let dependency = "export function add(a: number, b: number): number { return a + b; }";
        let out = run(consumer, &[("./m", dependency)]);
        assert!(out.contains("x + 1"), "marked imported call inlined (body spliced):\n{out}");
        assert!(out.contains("add(y, 2)"), "unmarked call to same import stays a call:\n{out}");
    }

    #[test]
    fn cross_file_callsite_inlines_marked_import_drops_import() {
        // When every call to the import is marked `/* @inline */`, all inline and
        // the now-unused import is dropped — the call-site analogue of the `@inline`
        // decl path's import cleanup.
        let consumer = r#"import { add } from "./m";
export function f(x: number): number { return /* @inline */ add(x, 1); }"#;
        let dependency = "export function add(a: number, b: number): number { return a + b; }";
        let out = run(consumer, &[("./m", dependency)]);
        assert!(!out.contains("add("), "the sole (marked) call inlined:\n{out}");
        assert!(!out.contains("import"), "now-unused import dropped:\n{out}");
        assert!(out.contains("x + 1"), "body spliced:\n{out}");
    }

    #[test]
    fn cross_file_callsite_inlines_marked_namespace_member() {
        // Namespace-member call-site: `/* @inline */ vec.scaleAndAdd(o, a, b, s)`
        // where `vec` is an imported namespace object. The member resolves through
        // the same registration the `@flatten` path uses, and a dep-forwarded const
        // (SCALE) folds — proving module deps are forwarded for the call-site path.
        let consumer = r#"import * as vec from "./v";
export function host(o: number[], a: number, b: number): number[] {
  return /* @inline */ vec.scaleAndAdd(o, a, b);
}"#;
        let dependency = "const SCALE = 2;\nexport function scaleAndAdd(out, a, b) { out[0] = a + b * SCALE; return out; }";
        let out = run(consumer, &[("./v", dependency)]);
        assert!(!out.contains("vec.scaleAndAdd"), "marked namespace member inlined:\n{out}");
        assert!(out.contains("a + b * 2"), "forwarded SCALE const folds into the body:\n{out}");
    }

    #[test]
    fn resolves_imported_interface_with_extends() {
        // The dependency's exported interface extends a local base; cross-module
...
/* @sroa */ export function f(): unknown { const v: Thing = mk(); return v[0]; }"#;
        let dependency = "export type Thing = { x: number };";
        let out = run(consumer, &[("./math", dependency)]);
...
export function f(r: number): number { return area(r); }"#;
    ...
    fn forwards_a_bare_specifier_import_dep() {
        // dependency body needs `clamp` from a bare package — forward verbatim.
        let consumer = r#"import { norm } from "./m";
export function f(x: number): number { return norm(x); }"#;
        let dependency = r#"import { clamp } from "math-utils";
/* @inline */ export function norm(v: number): number { return clamp(v); }"#;
    ...
    fn forwards_and_rebases_a_relative_import_dep() {
        // dependency at /proj/lib/m.ts imports ./util → /proj/lib/util; consumer at
        // /proj/app/entry.ts must see it as ../lib/util.
        let consumer = r#"import { norm } from "../lib/m";
export function f(x: number): number { return norm(x); }"#;
        let dependency = r#"import { clamp } from "./util";
/* @inline */ export function norm(v: number): number { return clamp(v); }"#;
        let dependencies = vec![Dependency {
        ...
        let out = transform_cross_file(consumer, &dependencies, &o, &mut crate::ModuleCache::new()).code;
    ...
    #[test]
    fn dedupes_a_forwarded_import_across_two_dependencies() {
        // two @inline dependencies from the same module both need `clamp` — forward once.
        let consumer = r#"import { lo, hi } from "./m";
export function f(x: number): number { return lo(x) + hi(x); }"#;
        let dependency = r#"import { clamp } from "pkg";
...
/* @inline */ export function hi(v: number): number { return clamp(v) + 1; }"#;
    ...
    // Barrel dependencies carry a `resolved` map so the core can follow re-exports by
    // path (mirrors what the plugin provides).
    fn barrel_dependency(specifier: &str, path: &str, code: &str, resolved: &[(&str, &str)]) -> Dependency {
        Dependency {
...
/* @inline */ export function add(a, b) { return a + b; }"#;
        let dependencies = vec![
            ...
            barrel_dependency("./vec3.js", "/n/mathcat/vec3.js", vec3, &[]),
        ];
        let out =
            transform_cross_file(consumer, &dependencies, &opts(), &mut crate::ModuleCache::new()).code;
...
export function f(x: number): number { return plus(x, 1); }"#;
        ...
        let dependencies = vec![
            ...
            barrel_dependency("./math.js", "/n/mathcat/math.js", math, &[]),
        ];
        let out =
            transform_cross_file(consumer, &dependencies, &opts(), &mut crate::ModuleCache::new()).code;
    ...
    fn inlines_a_default_function_export() {
        ...
        let consumer = r#"import add from "./m";
export function f(x: number): number { return add(x, 1); }"#;
        let dependency = "/* @inline */ export default function add(a: number, b: number): number { return a + b; }";
        let out = run(consumer, &[("./m", dependency)]);
    ...
    fn inlines_a_default_arrow_export() {
        let consumer = r#"import add from "./m";
export function f(x: number): number { return add(x, 1); }"#;
        let dependency = "/* @inline */ export default (a: number, b: number): number => a + b;";
        let out = run(consumer, &[("./m", dependency)]);
...
export function f(x: number): number { return vec.add(x, 1); }"#;
        let dependency = r#"/* @inline */ export default {
...
export function f(x: number): number { return add(x, 1) + sub(x, 2); }"#;
        let dependency = r#"/* @inline */ export default function add(a: number, b: number): number { return a + b; }
...
export function f(x: number): number { return vec3.add(x, 1); }"#;
        let dependency = r#"/* @inline */ export const vec3 = {
  add(a: number, b: number): number { return a + b; },
...
export function f(x: number): number { return vec.add(x, 1) + vec.sub(x, 2); }"#;
        let dependency = r#"export const vec = {
...
export function f(x: number): number { return vec.scale(x); }"#;
        let dependency = r#"const FACTOR = 3;
...
export function f(x: number): number { return plus(x, 1); }"#;
    ...
    fn inlines_through_a_renaming_export_clause() {
        // dependency exports a bare @inline fn under a different name via a clause.
        let consumer = r#"import { plus } from "./m";
export function f(x: number): number { return plus(x, 1); }"#;
        let dependency = "/* @inline */ function add(a: number, b: number): number { return a + b; }\nexport { add as plus };";
        let out = run(consumer, &[("./m", dependency)]);
...
export function f(x: number): number { return v.plus(x, 1); }"#;
    ...
    fn inlines_an_arrow_const_export() {
        ...
        let consumer = r#"import { add } from "./m";
export function f(x: number): number { return add(x, 1); }"#;
        let dependency = "/* @inline */ export const add = (a: number, b: number): number => a + b;";
        let out = run(consumer, &[("./m", dependency)]);
    ...
    fn inlines_a_function_expression_const_export() {
        let consumer = r#"import { add } from "./m";
export function f(x: number): number { return add(x, 1); }"#;
        let dependency = "/* @inline */ export const add = function(a: number, b: number): number { return a + b; };";
        let out = run(consumer, &[("./m", dependency)]);
    ...
    fn arrow_const_export_with_a_module_dep() {
        // arrow body referencing a module const exercises pull_dependency_deps + fold.
        let consumer = r#"import { scale } from "./m";
export function f(x: number): number { return scale(x); }"#;
...
export function f(x: number): number { return vec3.add(x, 1); }"#;
        let dependency = r#"/* @inline */ export function add(a: number, b: number): number { return a + b; }
/* @inline */ export function sub(a: number, b: number): number { return a - b; }"#;
        let out = run(consumer, &[("./m", dependency)]);
    ...
    fn namespace_member_call_with_a_module_dep() {
        // a namespace method needing a module const exercises pull_dependency_deps
...
export function mul(a: number, b: number): number { return a * b; }"#;
    ...
    #[test]
    fn alpha_renames_a_colliding_dependency_dep_const() {
        // SCALE repro (confirmed miscompile): the dependency's `bump` body needs the
        // dependency's `SCALE = 2`, but the consumer already declares a *different*
        // `SCALE = 100`. Copying the dependency const verbatim would make the spliced
        // body bind to the consumer's 100 → wrong (1001). The dependency dep must be
        // α-renamed so the body still uses the dependency's 2 → 21.
        let consumer = r#"import { bump } from "./dependency";
const SCALE = 100;
/* @optimize */ export function run() { return bump(10); }
export function useScale() { return SCALE; }"#;
        let dependency = "export const SCALE = 2;\n/* @inline */ export function bump(x) { return x * SCALE + 1; }";
        let out = run(consumer, &[("./dependency", dependency)]);
        assert!(!out.contains("bump("), "dependency call inlined:\n{out}");
        // The inlined `run` must compute 10*2+1 = 21 (dependency's SCALE), folded.
        assert!(out.contains("21"), "inlined body uses the dependency's SCALE (→21):\n{out}");
    ...
    #[test]
    fn no_rename_when_dependency_dep_name_is_free() {
        // Control: no collision (consumer has no `SCALE`) → the dependency dep keeps its
        // own name, no `$cf` suffix, and the body folds to the dependency's value.
        let consumer = r#"import { bump } from "./dependency";
/* @optimize */ export function run() { return bump(10); }"#;
        let dependency = "export const SCALE = 2;\n/* @inline */ export function bump(x) { return x * SCALE + 1; }";
        let out = run(consumer, &[("./dependency", dependency)]);
        assert!(!out.contains("$cf"), "no rename minted when there is no collision:\n{out}");
        assert!(out.contains("21"), "folds to the dependency's value:\n{out}");
    }
    ...
    fn alpha_renames_a_colliding_transitive_helper() {
        ...
        // the copied helper must rename consistently so the dependency's helper is used.
        let consumer = r#"import { bump } from "./dependency";
function helper(n) { return n + 1000; }
/* @optimize */ export function run() { return bump(10); }
export function useHelper() { return helper(0); }"#;
        let dependency = "function helper(n) { return n * 2; }\n/* @inline */ export function bump(x) { return helper(x); }";
        let out = run(consumer, &[("./dependency", dependency)]);
        assert!(!out.contains("bump("), "call inlined:\n{out}");
        // run() = helper_dependency(10) = 10*2 = 20, not the consumer's helper (1010).
        assert!(out.contains("20") || out.contains("10 * 2"), "dependency helper used:\n{out}");
        assert!(
    ...
    fn rebase_helper_cases() {
        ...
        // sibling of dependency, seen from a sibling consumer dir
        ...
        // parent traversal in the dependency spec
        assert_eq!(rebase_specifier("../core/x", "/proj/lib/m.ts", "/proj/lib/e.ts"), "../core/x");
    }

    // ── dependency_edges: the AST dependency-edge finder replacing the plugin's regexes ──

    fn edges(code: &str) -> Vec<String> {
        dependency_edges(code, SourceType::ts())
    }

    #[test]
    fn dependency_edges_sourced_reexports() {
    ...
    #[test]
    fn dependency_edges_namespace_barrel() {
    ...
    #[test]
    fn dependency_edges_rebind_shape() {
    ...
    #[test]
    fn dependency_edges_minified_and_asi() {
    ...
    #[test]
    fn dependency_edges_negative_plain_import_not_followed() {
    ...
    #[test]
    fn dependency_edges_dedup_and_order_stable() {
    ...
    // ── resolution_frontier: the demand-driven "what's still missing?" query ──

    /// Host id used across the frontier tests (its imports seed the frontier).
    const HOST: &str = "/app/entry.ts";

    fn frontier(host_code: &str, provided: &[Dependency]) -> Vec<(String, String, FrontierKind)> {
        resolution_frontier(HOST, host_code, provided, &[], &opts())
    }

    /// Frontier with a first-party `@inline`-def index (the build-start name set).
    fn frontier_with_defs(
        host_code: &str,
        provided: &[Dependency],
        inline_def_names: &[&str],
    ) -> Vec<(String, String, FrontierKind)> {
        let names: Vec<String> = inline_def_names.iter().map(|s| (*s).to_string()).collect();
        resolution_frontier(HOST, host_code, provided, &names, &opts())
    }

    /// A tagged expected request — `val("mathcat", HOST)` / `typ("./types.js", p)`.
    fn val(specifier: &str, from: &str) -> (String, String, FrontierKind) {
        (specifier.into(), from.into(), FrontierKind::Value)
    }
    fn typ(specifier: &str, from: &str) -> (String, String, FrontierKind) {
        (specifier.into(), from.into(), FrontierKind::Type)
    }

    #[test]
    fn frontier_directiveless_host_needs_nothing() {
        // No `@optimize`/`@flatten`/`@sroa` → nothing is inlined → no dependencies needed,
        // regardless of what the host imports and calls.
        let host = r#"import { vec3 } from "mathcat";
export function f(o: number[]): number[] { return vec3.set(o, 1, 2, 3); }"#;
        assert!(frontier(host, &[]).is_empty(), "directive-less host → no needs");
    }

    #[test]
    fn frontier_callsite_inline_of_imported_fn_requests_its_module() {
        // A host with ONLY a call-site `/* @inline */` of an imported function (no
        // `@optimize`/`@flatten`/`@sroa`) still has a value need: that call's module
        // must be gathered for the callee to inline. Before this wiring the frontier
        // returned `[]` (the gate ignored call-site spans); now it seeds the import
        // edge as a VALUE need, and once the module is provided, walks to empty.
        let host = r#"import { add } from "./m";
export function f(x: number): number { return /* @inline */ add(x, 1); }"#;
        // provided = [] → the seed edge `(./m, host)` is a VALUE need (the marked
        // call). It is also seeded as a TYPE need: the type phase scans every named
        // import unconditionally (over-inclusion is harmless — the module resolves
        // to no type and stops being requested once provided).
        assert_eq!(
            frontier(host, &[]),
            vec![val("./m", HOST), typ("./m", HOST)],
            "call-site @inline of an imported fn seeds its module (value + type)"
        );
        // provided = [the module declaring `add`] → `add` resolves locally, frontier empty.
        let m = barrel_dependency(
            "./m",
            "/n/m.js",
        ...
        assert!(
            frontier(host, &[m]).is_empty(),
            "callee module provided → nothing missing"
        );
    }

    #[test]
    fn frontier_namespace_barrel_walk_to_empty() {
        // The mathcat namespace-barrel shape: `import { vec3 } from "mathcat"` where
        // the index barrel does `export * as vec3 from './vec3.js'`. Walk the fixpoint
        // one module at a time, from bare (seed missing) → barrel provided (next hop
        // missing) → member module provided (frontier empty). `vec3` is used only as a
        // value here (no type annotation), but `@optimize` implies `@sroa`, so the type
        // phase also seeds `vec3` from `mathcat` → the seed module carries BOTH kinds
        // until it's provided (the value walk needs vec3.js, the type walk finds no
        // `Vec3`-shaped export there and stops).
        let host = r#"import { vec3 } from "mathcat";
/* @optimize */ export function host(o: number[]): number[] { return vec3.set(o, 1, 2, 3); }"#;

        // provided = [] → the seed edge `(mathcat, host)` is missing, as BOTH a value
        // need (the `vec3.set` call) and a type need (the `@sroa`-driven import scan).
        assert_eq!(
            frontier(host, &[]),
            vec![val("mathcat", HOST), typ("mathcat", HOST)],
            "seed edge to the directly-imported module, tagged value + type"
        );

        // provided = [mathcat index barrel] → the barrel does `export * as vec3 from
        // './vec3.js'`; the VALUE walk requests that re-export target. The TYPE walk of
        // `vec3` terminates in the barrel (an `export * as ns` namespace binding is not
        // a flat type name → no type request).
        let index = barrel_dependency(
            "mathcat",
            "/n/mathcat/index.js",
            r#"export * as vec3 from "./vec3.js";"#,
            &[("./vec3.js", "/n/mathcat/vec3.js")],
        );
        assert_eq!(
            frontier(host, std::slice::from_ref(&index)),
            vec![val("./vec3.js", "/n/mathcat/index.js")],
            "namespace re-export hop from the barrel (value only)"
        );

        // provided = [+ vec3.js] → `set` is a local callable of vec3.js → reachable,
        // frontier empty.
        let vec3 = barrel_dependency(
            "./vec3.js",
            "/n/mathcat/vec3.js",
        ...
        assert!(
            frontier(host, &[index, vec3]).is_empty(),
            "member module provided → nothing missing"
        );
    }

    #[test]
    fn frontier_quat_as_vec4_rebind_walk_to_empty() {
        // The quat-as-vec4 value re-bind: the host calls `quat.set`; quat's `set` is a
        // re-bound import (`import { set as set$1 } from './vec4.js'; const set = set$1;
        // export { set }`). Once mathcat-index + quat are provided, the frontier must
        // request vec4 (where the real `set` lives); after vec4 → empty.
        let host = r#"import { quat } from "mathcat";
/* @optimize */ export function host(o: number[]): number[] { return quat.set(o, 1, 2, 3, 4); }"#;

        let index = || {
            barrel_dependency(
                "mathcat",
                "/n/mathcat/index.js",
                r#"export * as quat from "./quat.js";"#,
                &[("./quat.js", "/n/mathcat/quat.js")],
            )
        };
        let quat = || {
            barrel_dependency(
                ...
                "/n/mathcat/quat.js",
                "import { set as set$1 } from './vec4.js';\nconst set = set$1;\nexport { set };",
                &[("./vec4.js", "/n/mathcat/vec4.js")],
            )
        };

        // index + quat provided → `set` in quat rebinds an import from './vec4.js',
        // which isn't provided → the VALUE walk requests it (from quat's path). The
        // TYPE walk of `quat` terminates in the `export * as quat` barrel (namespace
        // binding, not a flat type name).
        assert_eq!(
            frontier(host, &[index(), quat()]),
            vec![val("./vec4.js", "/n/mathcat/quat.js")],
            "rebound import hop to the origin module (value only)"
        );

        // + vec4 → `set` is a local callable there → reachable, frontier empty.
        let vec4 = barrel_dependency(
            "./vec4.js",
            "/n/mathcat/vec4.js",
            "export function set(out, x, y, z, w) { out[0] = x; out[1] = y; out[2] = z; out[3] = w; return out; }",
            &[],
        ...
        assert!(
            frontier(host, &[index(), quat(), vec4]).is_empty(),
            "origin module provided → nothing missing"
        );
    }

    #[test]
    fn frontier_fully_satisfiable_need_is_empty() {
        // A directly-imported callable already present in `provided` (no re-export
        // hops) → the seed is reachable and `set` is a local callable → nothing missing.
        let host = r#"import { set } from "./vec3.js";
/* @optimize */ export function host(o: number[]): number[] { return set(o, 1, 2, 3); }"#;
        let vec3 = barrel_dependency(
            "./vec3.js",
            "/n/mathcat/vec3.js",
            "export function set(out, x, y, z) { out[0] = x; out[1] = y; out[2] = z; return out; }",
        ...
        assert!(frontier(host, &[vec3]).is_empty(), "need fully satisfiable in provided → empty");
    }

    #[test]
    fn frontier_dedups_same_missing_specifier() {
        // Two host calls needing the same still-missing specifier collapse to ONE
        // value request (deduped, first-seen order). `@optimize` implies `@sroa`, so
        // `add`/`sub` are also scanned as types → one deduped TYPE request for the
        // same module (a value + a type request for `mathcat`, each once).
        let host = r#"import { add, sub } from "mathcat";
/* @optimize */ export function host(x: number): number { return add(x, 1) + sub(x, 2); }"#;
        assert_eq!(
            frontier(host, &[]),
            vec![val("mathcat", HOST), typ("mathcat", HOST)],
            "two value needs + two type needs on one missing module → one of each"
        );
    }

    #[test]
    fn frontier_type_need_walk_to_empty() {
        // The imported-type SROA path, mirroring the value walk. An `@optimize` host
        // (which implies `@sroa`) uses an imported `Vec3` in a `const v: Vec3 = mk()`
        // annotation SROA depends on; `mathcat`'s `.d.ts` index does `export * from
        // './types.js'`, and the shape is declared in `types.d.ts`. Walk the fixpoint:
        // bare → index provided → types provided (empty). The host calls `mk` too, so
        // `mk`'s value need seeds `mathcat` as well — hence a value AND a type request
        // until provided. (`@optimize` = `@flatten` + `@sroa`, exercising both phases.)
        let host = r#"import { Vec3, mk } from "mathcat";
/* @optimize */ export function host(): number { const v: Vec3 = mk(); v[0] = v[1]; return v[0]; }"#;

        // provided = [] → `mathcat` missing as a value (the `mk` call) and a type (`Vec3`).
        assert_eq!(
            frontier(host, &[]),
            vec![val("mathcat", HOST), typ("mathcat", HOST)],
            "seed edge missing as both a value and a type need"
        );

        // A package provided as BOTH its runtime `.js` and its `.d.ts` under ONE
        // specifier. The `.js` index carries no types (`Vec3` isn't there); the `.d.ts`
        // index re-exports the type module. `frontier_type_seed` must walk BOTH dependencies
        // sharing `mathcat` (not `.find` the `.js` and miss the `.d.ts`), so the type
        // request for `./types.js` is emitted from the `.d.ts` index.
        let index_js = || {
            barrel_dependency("mathcat", "/n/mathcat/index.js", "export function mk() { return [0, 0, 0]; }", &[])
        };
        let index_dts = || {
            barrel_dependency(
                "mathcat",
                "/n/mathcat/index.d.ts",
                "export * from \"./types.js\";\nexport declare function mk(): number[];",
                &[("./types.js", "/n/mathcat/types.d.ts")],
            )
        };
        // `mk` is now a local callable of index.js → value need satisfied. `Vec3` isn't
        // in either index; the `.d.ts` index re-exports it from `./types.js` (missing)
        // → one TYPE request to `./types.js` from the `.d.ts` index's path.
        assert_eq!(
            frontier(host, &[index_js(), index_dts()]),
            vec![typ("./types.js", "/n/mathcat/index.d.ts")],
            "type re-export hop from the `.d.ts` index (walks every specifier match)"
        );

        // + types.d.ts declaring the shape → `Vec3` resolves, frontier empty.
        let types_dts = barrel_dependency(
            "./types.js",
            "/n/mathcat/types.d.ts",
            "export type Vec3 = [number, number, number];",
            &[],
        );
        assert!(
            frontier(host, &[index_js(), index_dts(), types_dts]).is_empty(),
            "type-source module provided → nothing missing"
        );
    }

    #[test]
    fn frontier_inline_def_call_from_directiveless_host_requests_its_module() {
        // The C++-`inline`-style ergonomic: a DIRECTIVE-LESS host that merely CALLS a
        // first-party `@inline`-def name must still gather that def's module so the
        // `require_inline` path can inline the call. Without the `inline_def_names`
        // index the gate returns `[]` (no directive); with it, `helper`'s import edge
        // is a VALUE need.
        let host = r#"import { helper } from "./h";
export function f(x: number): number { return helper(x); }"#;

        // No index → directive-less host, nothing gathered (the old behavior).
        assert!(
            frontier(host, &[]).is_empty(),
            "directive-less host without the index → no needs"
        );

        // With the index → the seed edge `(./h, host)` is a VALUE need. It is also
        // seeded as a TYPE need: once the host is no longer short-circuited, the type
        // phase scans every named import unconditionally (over-inclusion is harmless —
        // the module resolves to no type and stops being requested once provided,
        // exactly as for a call-site `/* @inline */` host).
        assert_eq!(
            frontier_with_defs(host, &[], &["helper"]),
            vec![val("./h", HOST), typ("./h", HOST)],
            "an @inline-def call from a directive-less host seeds its module (value + type)"
        );

        // Provided the module whose `export function helper` is `@inline` → `helper`
        // resolves locally, frontier empty (fixpoint terminates).
        let h = barrel_dependency(
            "./h",
            "/app/h.ts",
            "/* @inline */ export function helper(a) { return a * 2; }",
            &[],
        );
        assert!(
            frontier_with_defs(host, &[h], &["helper"]).is_empty(),
            "def module provided → nothing missing"
        );
    }

    #[test]
    fn frontier_inline_def_index_uncalled_name_needs_nothing() {
        // The index is a per-file gate: a name in `inline_def_names` the host does NOT
        // call must not fire the frontier (a directive-less non-caller stays a no-op).
        let host = r#"import { other } from "./h";
export function f(x: number): number { return other(x); }"#;
        assert!(
            frontier_with_defs(host, &[], &["helper"]).is_empty(),
            "host calls `other`, index has `helper` → no needs"
        );
    }

    #[test]
    fn frontier_inline_def_call_and_inliner_splices_body() {
        // End to end at the core level: with `helper` in the index, a directive-less
        // host's `helper(` call gathers `./h` (frontier), and the UNCHANGED
        // `transform_cross_file` `require_inline` path then inlines the call — the
        // consumer had NO directive.
        let host = r#"import { helper } from "./h";
export function f(x: number): number { return helper(x) + 1; }"#;
        let h = barrel_dependency(
            "./h",
            "/app/h.ts",
            "/* @inline */ export function helper(a) { return a * 2; }",
        ...
        // Frontier says: gather `./h` (value + the harmless type over-scan).
        assert_eq!(
            frontier_with_defs(host, &[], &["helper"]),
            vec![val("./h", HOST), typ("./h", HOST)]
        );
        // Provided it, the inliner splices the body (no `helper(` call survives).
        let out = transform_cross_file(host, &[h], &opts(), &mut crate::ModuleCache::new()).code;
        assert!(!out.contains("helper("), "helper call inlined into directive-less host:\n{out}");
        assert!(out.contains("x * 2"), "helper body spliced:\n{out}");
    }
}
