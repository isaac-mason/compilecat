...
describe('plugin-alt/transforms/inline — regression guards', () => {
	it('expression-position multi-stmt body: hoists pure prelude and splices return expr', () => {
		...
		// end up inside the pick body. First splice gets bare names; second
		// collides and gets `_2` suffixes.
		expect((pickBody.match(/\bconst (dx|dx_2)\s*=/g) ?? []).length).toBe(2);
		expect((pickBody.match(/\bconst (dy|dy_2)\s*=/g) ?? []).length).toBe(2);
		expect((pickBody.match(/\bconst (dz|dz_2)\s*=/g) ?? []).length).toBe(2);
	});
	...
	it('mutated param via AssignmentExpression: hoisted to a `let` temp', () => {
		...
		// a `let _arg_rad` (no collision, so no numeric suffix) and the `*= 0.5`
		// lands on it.
		expect(workBody).toMatch(/let _arg_rad\s*=\s*angle/);
		expect(workBody).toMatch(/_arg_rad\s*\*=\s*0\.5/);
	});
	...
	it('mutated param via UpdateExpression: hoisted to a `let` temp', () => {
		...
		expect(workBody).toMatch(/let _arg_i\s*=\s*start/);
		expect(workBody).toMatch(/_arg_i\+\+/);
	});
	...
	it('pure-init check accepts nested member-chain computed keys', () => {
		...
		expect(workBody).toMatch(/const node\s*=\s*dbvt\.nodes\[root\]/);
		expect(workBody).toMatch(/const parent\s*=\s*dbvt\.nodes\[node\.parent\]/);
	});

	it('renames callee local that collides with an existing caller local', () => {
		// Caller already has `node` bound — splicing a callee that declares
		// `node` must rename to avoid shadowing the caller's binding.
		const input = `
			/* @inline */
			function lookup(tree, idx) {
				const node = tree.nodes[idx];
				return node.value;
			}
			export function work(tree, root) {
				const node = tree.root;
				const v = lookup(tree, root);
				return node.tag + v;
			}
		`;
		const out = run(input);
		const workBody = out.slice(out.indexOf('function work'));
		expect(workBody).not.toMatch(/\blookup\(/);
		// Caller's original `node` must stay bare; callee's `node` is renamed.
		expect(workBody).toMatch(/const node\s*=\s*tree\.root/);
		expect(workBody).toMatch(/const node_2\s*=\s*tree\.nodes\[root\]/);
	});

	it('renames callee local that collides with a caller param', () => {
		const input = `
			/* @inline */
			function scale(v, k) {
				const tmp = v * k;
			...
			export function work(tmp, k) {
				return scale(tmp, k);
			}
		`;
		const out = run(input);
		const workBody = out.slice(out.indexOf('function work'));
		expect(workBody).not.toMatch(/\bscale\(/);
		// Callee's `tmp` must rename because the caller has a `tmp` param.
		expect(workBody).toMatch(/const tmp_2\s*=\s*tmp\s*\*\s*k/);
	});

	it('skips over existing name_N when picking a fresh suffix', () => {
		// Caller already has both `tmp` and `tmp_2` in scope — the spliced
		// `tmp` must become `tmp_3`, not collide with either.
		const input = `
			/* @inline */
			function step(x) {
				const tmp = x + 1;
			...
			export function work(x) {
				const tmp = 1;
				const tmp_2 = 2;
				return step(x) + tmp + tmp_2;
			}
		`;
		const out = run(input);
		const workBody = out.slice(out.indexOf('function work'));
		expect(workBody).not.toMatch(/\bstep\(/);
		expect(workBody).toMatch(/const tmp_3\s*=\s*x\s*\+\s*1/);
	});
